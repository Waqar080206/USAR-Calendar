...
function buildInitialState(
  ...
  const dateKey = isValidDateKey(rawDate) ? (rawDate as string) : todayKey;
    ...
    <div className="pb-24 lg:pb-8">
      <div className="mx-auto max-w-[1600px] px-4 py-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-3">
          ...
          <div className="space-y-6 lg:col-span-2">
            ...
            <div className="rounded-lg border border-slate-200 bg-white p-8">
              <p className="text-center text-slate-500">
                {state.view === "month"
                  ? `${format(monthDate, "MMMM yyyy")} view`
                  : state.view === "week"
          ...
          {/* Right sidebar - Event details and filters */}
          <div className="space-y-6">
            ...
            {selectedItem && (
              <div className="rounded-lg border border-slate-200 bg-white p-4">
                <div
                  className={`inline-block rounded-full px-2.5 py-1 text-xs font-semibold ${eventTypeMeta[selectedItem.type].chipClass} mb-3`}
                >
                  {eventTypeMeta[selectedItem.type].label}
                </div>
                <h4 className="mb-2 font-semibold text-slate-900">
                  {selectedItem.title}
                </h4>
                <p className="mb-3 text-sm text-slate-600">
                  {getDisplayRange(selectedItem)}
                </p>
            ...
            <div className="rounded-lg border border-slate-200 bg-white p-4">
              ...
              <p className="text-xs text-slate-600">
                {stats.elapsedDays} of {stats.totalDays} days completed
              </p>
            </div>
          </div>
        </div>
